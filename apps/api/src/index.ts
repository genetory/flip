  ...
  async (req, res) => {
    ...
    try {
      ...
      const systemPrompt =
        ...
        "3. 대화에서 학생이 '알게 된/정리할 만한' 핵심 포인트를 materials 배열에 간결한 한 줄로 담아(예: '백엔드 개발자: 서버·API 설계·구현이 주 업무, Java/Spring·DB 지식이 핵심'). 이전 것도 유지해 매 턴 누적 반환.\n" +
        "4. 고른 직무들을 두루 다뤄 핵심 포인트가 5개 이상 모이면 done=true, 따뜻한 마무리와 함께 '다음 주엔 이 방향으로 이력서를 만든다'는 안내를 reply 에 담아. 얕으면 done 을 서두르지 마.\n" +
