...
const CAREER_PROMPTS: Record<string, { label: string; week: number; step: string; default: string }> = {
  ...
  interview_self: {
    ...
    step: "스텝 2 · 자기소개 면접",
    default:
      "[이번 면접: 자기소개 면접] 면접 초반 라운드처럼 진행한다. 반드시 학생이 실제로 작성한 [이력서]와 [자기소개서]의 내용을 근거로 질문해 — 일반적인 자기소개 질문을 던지지 말고, 학생이 쓴 문장·경험을 직접 인용하거나 언급하며 파고들어. 예: 자기소개서의 지원 동기·성장 과정에서 '이렇게 쓰셨는데 그 계기가 무엇이었나요?', 이력서의 특정 활동·경력을 짚으며 '여기서 맡은 역할을 더 설명해 주세요'. 1분 자기소개, 지원 동기, 성격·강점, 성장 배경 등 '이 사람이 누구인지'를 그 사람이 쓴 내용을 바탕으로 확인해. 직무 기술 심화나 압박 질문은 이번 라운드에서 다루지 마. 이력서·자소서가 비어 있으면 먼저 일반적인 자기소개로 시작하되 답변에서 나온 내용을 이어서 파고들어."
  },
  interview_job: {
    label: "모의면접 · 직무 면접",
    week: 4,
