...
export const CULTURE_LESSONS: Record<string, CultureLesson> = {
  w1s4: {
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          { title: "나만의 강점", body: "다국어 능력·글로벌 감각·여러 문화에 대한 이해는 한국인 지원자에게 없는 큰 무기예요. 해외 시장을 넓히거나 외국 고객·파트너를 상대하는 직무에서 특히 빛나니, 이 강점을 자기소개서와 면접에서 구체적인 사례로 보여주세요. '나라서 이 회사에 도움이 되는 이유'로 연결하면 설득력이 커져요.", example: "예) '베트남 시장 진출을 준비하는 팀이라면, 베트남어와 현지 문화를 이해하는 제가 고객 소통에 기여할 수 있습니다'처럼 강점을 직무와 연결해요." },
  ...
  "w4-apply": {
    ...
    emoji: "🚀",
    title: "스스로 지원하는 법",
    intro: "이력서·자기소개서·면접 준비까지 마쳤다면, 이제 직접 지원할 차례예요. 외국인 유학생이 어디서 공고를 찾고, 어떤 회사가 비자 지원에 열려 있으며, 어떻게 전략적으로 지원하는지 정리했어요. 마지막 퀴즈로 확인해봐요.",
    objectives: [
      "외국인이 지원할 채용 공고를 어디서 찾는지 알아요",
      "E-7 등 취업 비자 지원에 열린 기업을 가려내는 법을 알아요",
      "합격률을 높이는 지원 전략과 순서를 이해해요",
      "지원 전 마지막으로 점검할 체크리스트를 갖춰요"
    ],
    sections: [
      {
        heading: "1. 공고는 여기서 찾아요",
        emoji: "🔎",
        summary: "채널마다 강점이 달라요. 두세 곳을 정해 꾸준히 보는 게 핵심이에요.",
        items: [
          { title: "종합 채용 플랫폼", body: "사람인·잡코리아·원티드·링크드인이 대표적이에요. 원티드와 링크드인은 스타트업·외국계·IT 직군이 많고 영어 공고도 자주 보여, 외국인 지원자에게 특히 유리해요. 관심 직무 키워드로 알림을 설정해두면 새 공고를 놓치지 않아요.", tip: "'글로벌', 'English', '외국인' 키워드로 검색하면 문턱이 낮은 공고가 잘 걸려요." },
          { title: "외국인·유학생 특화 채널", body: "외국인 채용을 전문으로 다루는 채널과 대학의 국제처·커리어센터 공고도 놓치지 마세요. 학교 커리어센터는 유학생을 위한 채용설명회·인턴 정보를 먼저 안내하는 경우가 많아, 등록해두면 좋은 기회를 앞서 받을 수 있어요.", tip: "재학 중이라면 학교 국제처·커리어센터 메일링을 꼭 신청해두세요." },
          { title: "회사 채용 페이지 직접 지원", body: "가고 싶은 회사가 뚜렷하다면 그 회사의 채용(Careers) 페이지에서 직접 지원하는 것도 좋아요. 플랫폼에 올라오지 않는 상시 채용이 있는 경우가 많고, 회사에 대한 관심을 더 잘 보여줄 수 있어요.", example: "예) 관심 기업 5곳을 정해 채용 페이지를 즐겨찾기하고, 2주에 한 번씩 새 공고를 확인해요." },
          { title: "채용 박람회·네트워킹", body: "외국인 대상 잡페어(K-Move, 외국인 채용 박람회 등)와 링크드인 네트워킹도 좋은 통로예요. 현직자에게 직무를 물어보거나 추천(리퍼럴)을 받으면 서류 통과 확률이 눈에 띄게 올라가요.", tip: "링크드인에서 같은 직무의 현직자에게 예의 바르게 메시지를 보내 커피챗을 청해보세요." }
        ]
      },
      {
        heading: "2. 비자 지원에 열린 회사 가려내기",
        emoji: "🛂",
        summary: "외국인 채용은 '비자 스폰서'가 핵심이에요. 지원 전에 꼭 확인해요.",
        items: [
          { title: "E-7(특정활동) 비자란", body: "유학생이 졸업 후 전문직으로 취업할 때 가장 흔한 취업 비자가 E-7이에요. 전공·경력과 관련된 직무여야 하고, 회사가 비자 발급에 필요한 서류를 함께 준비해줘야 해요. 그래서 '외국인 채용 경험이 있는 회사'인지가 중요해요.", tip: "전공과 직무의 연관성이 클수록 E-7 승인에 유리해요 — 그래서 1주차 직무 선정이 중요했어요." },
          { title: "공고에서 신호 읽기", body: "공고에 '외국인 지원 가능', '비자 스폰서십 제공', 'visa support' 같은 문구가 있으면 초록불이에요. 반대로 '내국인만', '병역 필' 같은 조건이 있으면 지원이 어려울 수 있어요. 애매하면 지원 전 채용 담당자에게 정중히 문의해도 괜찮아요.", example: "문의 예시:\n'해당 포지션이 외국인(E-7 비자) 지원도 가능한지 여쭤보고 싶습니다.'" },
          { title: "외국인 채용에 열린 곳", body: "외국계 기업, 글로벌 사업을 하는 IT·스타트업, 무역·물류, 다국어가 필요한 직무는 상대적으로 외국인 채용에 열려 있어요. 내 언어·문화 강점이 '있으면 좋은' 게 아니라 '필요한' 자리를 노리면 경쟁력이 커져요.", tip: "다국어·현지화·글로벌 세일즈 직무는 유학생의 강점이 곧 채용 이유가 돼요." },
          { title: "비자 요건 미리 확인", body: "지원과 동시에 내 비자 상태(체류 기간, D-2/D-10 등)와 E-7 전환 요건을 미리 확인해두면 합격 후 절차가 매끄러워요. 하이코리아(HiKorea)와 출입국·외국인청 안내로 필요한 서류를 파악해두세요.", tip: "졸업 예정이라면 구직(D-10) 비자로 체류 기간을 확보하는 방법도 알아두세요." }
        ]
      },
      {
        heading: "3. 전략적으로 지원해요",
        emoji: "🎯",
      ...
      {
        heading: "4. 지원 전 마지막 체크리스트",
        emoji: "✅",
        summary: "제출 버튼을 누르기 전, 이 항목들을 확인해요.",
        items: [
          { title: "서류 점검", body: "이력서·자기소개서에 오탈자나 예전 회사명이 남아있지 않은지, 공고 직무에 맞게 손봤는지 확인해요. 파일명은 '이름_직무_이력서'처럼 알아보기 쉽게 정하고, PDF로 저장해 형식이 깨지지 않게 보내요.", tip: "제출 전 소리 내어 한 번 읽으면 어색한 문장·오타가 잘 보여요." },
          { title: "자격요건 대조", body: "공고의 필수 자격요건을 내가 충족하는지, 우대사항 중 강조할 만한 게 있는지 한 번 더 대조해요. 부족한 요건이 있어도 그걸 보완할 강점을 지원서에 드러내면 돼요." },
          { title: "비자·연락처 확인", body: "외국인 지원 가능 여부와 비자 요건을 확인했는지, 이력서의 이메일·전화번호가 정확한지 점검해요. 연락이 닿지 않아 기회를 놓치는 일이 없도록 해요.", tip: "면접 연락을 놓치지 않게 알림·메일을 자주 확인하세요." },
          { title: "마감·후속 준비", body: "마감일 전에 여유 있게 제출하고, 지원 후에는 면접 예상 질문(2단계)과 모의면접(3단계)으로 다음 단계를 준비해요. 지원은 끝이 아니라 면접 준비의 시작이에요." }
        ]
      }
    ],
    quiz: [
      {
        question: "외국인 유학생이 공고를 찾기에 특히 유리한 채널은?",
        options: ["아무 곳이나 상관없다", "원티드·링크드인 등 IT·외국계가 많은 채널", "지원하지 않고 기다린다", "종이 신문 광고"],
        answer: 1,
        explain: "원티드·링크드인은 스타트업·외국계·영어 공고가 많아 외국인 지원자에게 유리해요."
      },
      {
        question: "공고에서 '초록불' 신호로 볼 수 있는 문구는?",
        options: ["내국인만 지원", "비자 스폰서십 제공 / visa support", "병역 필", "해당 없음"],
        answer: 1,
        explain: "'외국인 지원 가능', '비자 스폰서십 제공' 같은 문구는 외국인 채용에 열려 있다는 신호예요."
      },
      {
        question: "합격률을 높이는 지원 방식은?",
        options: ["같은 서류를 최대한 많이 뿌린다", "공고 키워드에 맞춰 지원 동기·강점을 손본다", "한 회사만 계속 지원한다", "마감 직전에 급하게 낸다"],
        answer: 1,
        explain: "공고의 자격요건에 맞춰 서류를 맞춤화하면 통과율이 올라가요."
      },
      {
        question: "유학생이 취업 시 흔히 전환하는 취업 비자는?",
        options: ["D-2(유학)", "E-7(특정활동)", "B-2(관광)", "없음"],
        answer: 1,
        explain: "전공·직무와 관련된 전문직 취업 시 E-7 비자가 가장 일반적이며, 회사의 지원이 필요해요."
      }
    ]
  }
};
