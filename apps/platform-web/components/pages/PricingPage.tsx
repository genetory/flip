...
export function PricingPage() {
  ...
  const t = (ko: string, en: string, zh?: string, vi?: string) => (isKo ? ko : isZh ? (zh ?? en) : isVi ? (vi ?? en) : en);
  ...
  const copy = {
    ...
    refundCard: {
      title: t("탈락 시 100% 환불", "100% Refund if Not Accepted", "未通过 100% 退款", "Hoàn 100% nếu không được chọn"),
      subtitle: t("부담 없이 도전하세요", "Apply with confidence", "无负担参与", "Hãy ứng tuyển một cách tự tin"),
      description: t(
      ...
      processSteps: [
        { label: t("결제 완료", "Payment complete", "完成付款", "Hoàn tất thanh toán"), imageSrc: "/img_process_0.webp" },
        { label: t("프로필 제출", "Profile submission", "提交资料", "Nộp hồ sơ"), imageSrc: "/img_process_1.webp" },
        { label: t("심사 진행", "Screening", "审核进行中", "Đang thẩm định"), imageSrc: "/img_process_2.webp" },
        { label: t("프로그램 참여", "Program participation", "参与项目", "Tham gia chương trình"), imageSrc: "/img_process_3.webp" }
      ]
    },
    housing: {
      titleTop: t("집 구하기 걱정 없이,", "Focus on your career,", "无需为找房烦恼,", "Đừng lo chuyện tìm nhà,"),
      titleBottom: t("커리어에만 집중하세요.", "not on finding housing.", "只需专注于职业发展。", "hãy tập trung vào sự nghiệp."),
      description: t(
      ...
      privateRoom: {
        name: t("Private Room (1인실)", "Private Room (Single)", "Private Room (单人间)", "Private Room (Phòng đơn)"),
        location: t(
          "📍 서울 내 위치, 교통 편리",
      ...
      sharedRoom: {
        name: t("Shared Room (2인실)", "Shared Room (Double)", "Shared Room (双人间)", "Shared Room (Phòng đôi)"),
        location: t(
          "📍 서울 내 위치, 교통 편리",
          "📍 Located in Seoul with convenient transportation",
          "📍 位于首尔,交通便利",
            ...
            <div className="mx-auto mt-10 w-full max-w-4xl space-y-10">
              <div className="w-full">
                <Image
                  ...
                  alt={t("일경험 소개 이미지", "Work experience hero image", "工作经验介绍图片", "Hình ảnh giới thiệu trải nghiệm công việc")}
                  width={1600}
                  height={900}
              ...
              <div className="grid w-full gap-3 md:grid-cols-2 md:items-stretch">
                  <article className="flex h-full w-full flex-col rounded-2xl bg-white p-5 text-[#111111] shadow-card md:p-6">
                    <span className="mb-3 inline-flex w-fit items-center rounded-full bg-[#B7FF5A] px-2.5 py-1 text-[11px] font-semibold text-black">
                      {t("진행중", "In Progress", "进行中", "Đang triển khai")}
                    </span>
                    <h4 className={`${paperlogy.className} text-xl font-black tracking-[-0.02em] md:text-2xl`}>{t("해외 대학 출신 일경험", "Work Experience for Overseas Graduates", "海外大学毕业生工作经验", "Trải nghiệm việc làm cho cử nhân từ nước ngoài")}</h4>
                    <p className="mt-1 text-sm font-medium text-foreground/90">{t("해외 대학 출신을 위한 한국 기업 일경험", "Korean-company work experience for overseas graduates", "面向海外大学毕业生的韩国企业工作体验", "Trải nghiệm làm việc tại doanh nghiệp Hàn Quốc dành cho cử nhân nước ngoài")}</p>
                    ...
                    <div className="mt-4 overflow-x-auto rounded-xl bg-white/92">
                      <table className="w-full min-w-[320px] border-collapse text-left text-sm">
                        <tbody>
                          <tr className="border-b border-border/60"><th className="w-24 bg-muted/30 px-3 py-2 font-semibold">{t("대상", "Target", "对象", "Đối tượng")}</th><td className="px-3 py-2">{t("해외 대학 출신", "Overseas university graduates", "海外大学毕业生", "Cử nhân từ đại học nước ngoài")}</td></tr>
                          <tr className="border-b border-border/60"><th className="bg-muted/30 px-3 py-2 font-semibold">{t("목적", "Purpose", "目的", "Mục đích")}</th><td className="px-3 py-2">{t("한국 기업 일경험 및 직무 이해", "Korean-company work experience and role understanding", "韩国企业工作经验及岗位理解", "Trải nghiệm làm việc và hiểu biết về vị trí tại doanh nghiệp Hàn Quốc")}</td></tr>
                          <tr className="border-b border-border/60"><th className="bg-muted/30 px-3 py-2 font-semibold">{t("급여", "Salary", "薪资", "Lương")}</th><td className="px-3 py-2">{t("없음", "None", "无", "Không")}</td></tr>
                          <tr className="border-b border-border/60"><th className="bg-muted/30 px-3 py-2 font-semibold">{t("참여비", "Applicant fee", "参与费", "Phí tham gia")}</th><td className="px-3 py-2">2,000,000원</td></tr>
                          <tr className="border-b border-border/60"><th className="bg-muted/30 px-3 py-2 font-semibold">{t("기업 수수료", "Company fee", "企业费用", "Phí doanh nghiệp")}</th><td className="px-3 py-2">{t("없음", "None", "无", "Không")}</td></tr>
                          <tr className="border-b border-border/60"><th className="bg-muted/30 px-3 py-2 font-semibold">{t("수료증", "Certificate", "结业证书", "Chứng chỉ")}</th><td className="px-3 py-2">{t("발급", "Provided", "提供", "Cấp")}</td></tr>
                          <tr><th className="bg-muted/30 px-3 py-2 font-semibold">{t("전환 가능성", "Conversion", "转正可能性", "Khả năng chuyển đổi")}</th><td className="px-3 py-2">{t("전환 가능, 보장하지 않음", "Possible, not guaranteed", "可转正，不保证", "Có thể chuyển đổi, không đảm bảo")}</td></tr>
                        </tbody>
                      </table>
                    </div>
                    <div className="mt-4">
                      <p className="text-sm font-semibold">{t("제공 내용", "What is provided", "提供内容", "Nội dung được cung cấp")}</p>
                      <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-muted-foreground">
                        <li>{t("직무 교육 제공 (플리퍼스)", "Job orientation/training (Flipers)", "职务培训提供 (Flipers)", "Đào tạo nghiệp vụ (Flipers)")}</li>
                        <li>{t("수료증 발급 (플리퍼스)", "Completion certificate (Flipers)", "结业证书发放 (Flipers)", "Cấp chứng chỉ hoàn thành (Flipers)")}</li>
                        <li>{t("정기 면담, 과제, 피드백 (기업 협조 기반)", "Periodic check-ins, tasks, and feedback (company-supported)", "定期面谈、任务、反馈（基于企业配合）", "Gặp mặt định kỳ, nhiệm vụ, phản hồi (dựa trên hỗ trợ doanh nghiệp)")}</li>
                        <li>{t("추천서 제공은 기업 희망 시 자율 진행", "Recommendation letter only if the company chooses to provide it", "推荐信由企业自主决定是否提供", "Thư giới thiệu được cung cấp tùy doanh nghiệp")}</li>
                      </ul>
                    </div>
                  ...
                  <article className="h-full w-full rounded-2xl bg-white p-5 text-[#111111] shadow-card md:p-6">
                    ...
                    <h4 className={`${paperlogy.className} text-xl font-black tracking-[-0.02em] md:text-2xl`}>{t("국내 대학 재학생 및 졸업생 일경험", "Work Experience for Korean-University Students/Graduates", "韩国大学在读生及毕业生的工作体验", "Trải nghiệm việc làm cho sinh viên và cựu sinh viên đại học Hàn Quốc")}</h4>
                    <p className="mt-1 text-sm font-medium text-foreground/90">{t("국내 대학 유학생을 위한 실무 일경험", "Practical work experience for international students in Korean universities", "面向韩国大学留学生的实务工作体验", "Trải nghiệm làm việc thực tế dành cho du học sinh tại các trường đại học Hàn Quốc")}</p>
                    <p className="mt-3 text-xs leading-relaxed text-muted-foreground md:text-sm">
                      {t(
                    ...
                    <div className="mt-4 overflow-x-auto rounded-xl bg-white/96 text-[#1f2342]">
                      <table className="w-full min-w-[320px] border-collapse text-left text-sm">
                        <tbody>
                          <tr className="border-b border-border/60"><th className="w-24 bg-muted/30 px-3 py-2 font-semibold">{t("대상", "Target", "对象", "Đối tượng")}</th><td className="px-3 py-2">{t("국내 대학 재학생 및 졸업생", "Students/graduates of Korean universities", "韩国大学在读生及毕业生", "Sinh viên và cựu sinh viên đại học Hàn Quốc")}</td></tr>
                          <tr className="border-b border-border/60"><th className="bg-muted/30 px-3 py-2 font-semibold">{t("목적", "Purpose", "目的", "Mục đích")}</th><td className="px-3 py-2">{t("한국 기업 일경험 및 직무 이해", "Korean-company work experience and role understanding", "韩国企业工作经验及岗位理解", "Trải nghiệm làm việc và hiểu biết về vị trí tại doanh nghiệp Hàn Quốc")}</td></tr>
                          <tr className="border-b border-border/60"><th className="bg-muted/30 px-3 py-2 font-semibold">{t("급여", "Salary", "薪资", "Lương")}</th><td className="px-3 py-2">{t("없음", "None", "无", "Không")}</td></tr>
                          <tr className="border-b border-border/60"><th className="bg-muted/30 px-3 py-2 font-semibold">{t("참여비", "Applicant fee", "参与费", "Phí tham gia")}</th><td className="px-3 py-2">700,000원</td></tr>
                          <tr className="border-b border-border/60"><th className="bg-muted/30 px-3 py-2 font-semibold">{t("기업 수수료", "Company fee", "企业费用", "Phí doanh nghiệp")}</th><td className="px-3 py-2">{t("없음", "None", "无", "Không")}</td></tr>
                          <tr className="border-b border-border/60"><th className="bg-muted/30 px-3 py-2 font-semibold">{t("수료증", "Certificate", "结业证书", "Chứng chỉ")}</th><td className="px-3 py-2">{t("발급", "Provided", "提供", "Cấp")}</td></tr>
                          <tr><th className="bg-muted/30 px-3 py-2 font-semibold">{t("학교 연계", "School linkage", "校方对接", "Liên kết với nhà trường")}</th><td className="px-3 py-2">{t("평가서/서류 작성 조율 가능", "Evaluation/report documents can be coordinated", "可协调评估书/文件", "Có thể phối hợp giấy đánh giá/hồ sơ")}</td></tr>
                        </tbody>
                      </table>
                    </div>
                    <div className="mt-4">
                      ...
                      <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-muted-foreground">
                        <li>{t("직무 교육 제공 (플리퍼스)", "Job orientation/training (Flipers)", "职务培训提供 (Flipers)", "Đào tạo nghiệp vụ (Flipers)")}</li>
                        <li>{t("수료증 발급 (플리퍼스)", "Completion certificate (Flipers)", "结业证书发放 (Flipers)", "Cấp chứng chỉ hoàn thành (Flipers)")}</li>
                        <li>{t("학교 연계 시 평가서/학점 서류 요청 조율", "School-linked evaluation/credit documents can be coordinated", "校方对接时可协调评估书/学分文件", "Có thể phối hợp giấy đánh giá/học phần khi liên kết với trường")}</li>
                        <li>{t("기업 수수료 없음, 기업은 운영 협조 항목만 선택 참여", "No company fee; companies participate through selected cooperation items only", "无企业费用，企业仅选择性参与运营配合", "Không thu phí doanh nghiệp, doanh nghiệp chỉ tham gia các hạng mục hợp tác đã chọn")}</li>
                      </ul>
                    </div>
                    <p className="mt-4 rounded-md bg-amber-50 px-3 py-2 text-xs leading-relaxed text-amber-900">
                      {t(
              ...
              <article className="w-full rounded-2xl bg-white p-6 text-[#111111] shadow-card">
                <div className="flex items-center justify-between gap-3">
                  <h3 className={`${paperlogy.className} text-xl font-black tracking-[-0.02em] text-slate-900 md:text-2xl`}>
                    {t("인턴십 · 계약직 · 정규직", "Internship · Contract · Regular", "实习 · 合同 · 正职", "Thực tập · Hợp đồng · Chính thức")}
                  </h3>
                  <span className="rounded-full bg-[#0B46E8] px-3 py-1 text-xs font-semibold text-white">
                    {t("계획중", "Planned", "计划中", "Đang lên kế hoạch")}
                  </span>
                </div>
