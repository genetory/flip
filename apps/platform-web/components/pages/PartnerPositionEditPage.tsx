...
export function PartnerPositionEditPage({ positionId }: { positionId: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (ignore) return;
        setTitle(item.title ?? "");
        setStatus(item.status === "DRAFT" ? "DRAFT" : "OPEN");
  ...
  }, [isAuthenticated, isReady, positionId, user]);
    ...
    <div className="min-h-screen flex flex-col bg-background font-sans text-foreground antialiased">
      ...
      <main className="container py-12 md:py-16">
        <div className="mx-auto max-w-4xl">
          <h1 className="mb-6 font-display text-3xl font-bold tracking-tight">{t("포지션 수정하기", "Edit position")}</h1>
            ...
            <section className="space-y-6">
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <Button variant="dark" onClick={() => void handleSubmit()} disabled={isSubmitting}>
                  {isSubmitting ? t("수정 중...", "Saving...") : t("포지션 수정", "Save changes")}
                </Button>
              </div>
            </section>
          )}
