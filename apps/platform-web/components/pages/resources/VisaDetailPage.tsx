...
export function VisaDetailPage({ code }: Props) {
  ...
  const pickVisaLabel = (item: NonNullable<ReturnType<typeof getFlatVisaItems>[number]>) =>
    locale === "ko" ? item.labelKo : locale === "zh-CN" ? item.labelZh : locale === "vi" ? item.labelVi : item.labelEn;
  const pickVisaApplicant = (item: NonNullable<ReturnType<typeof getFlatVisaItems>[number]>) =>
    locale === "ko" ? item.applicantKo : locale === "zh-CN" ? item.applicantZh : locale === "vi" ? item.applicantVi : item.applicantEn;
  const pickVisaDuration = (item: NonNullable<ReturnType<typeof getFlatVisaItems>[number]>) =>
    locale === "ko" ? item.durationKo : locale === "zh-CN" ? item.durationZh : locale === "vi" ? item.durationVi : item.durationEn;
    ...
    return (
      ...
      >
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          <p className="text-sm text-slate-600">{t("해당 코드가 목록에 없어요.", "This code is not in the list.", "该代码不在列表中。", "Mã này không có trong danh sách.")}</p>
          <Link href="/resources/visa" className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-[#0B46E8] hover:underline">
            ...
            {t("비자 목록으로 돌아가기", "Back to visa list", "返回签证列表", "Quay lại danh sách visa")}
          </Link>
        </section>
      </ResourceSubPageLayout>
    );
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 md:p-7">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-700 md:text-base">
            <span className="font-semibold">{t("신청대상", "Applicants", "申请对象", "Đối tượng nộp hồ sơ")}:</span> {pickVisaApplicant(visa)}
          </p>
          <p className="mt-1 text-sm text-slate-700 md:text-base">
            <span className="font-semibold">{t("체류기간", "Stay period", "停留期限", "Thời gian lưu trú")}:</span> {pickVisaDuration(visa)}
          </p>
        </div>
        ...
        <div className="mt-5 border-t border-slate-200 pt-5">
          <div className="space-y-5">
            ...
            <section className="rounded-2xl bg-slate-50 p-4">
              <h3 className="text-sm font-black text-[#0B1227] md:text-base">{t("한눈에 보기", "Quick Overview", "快速概览", "Tổng quan nhanh")}</h3>
              <ul className="mt-2 space-y-1 text-sm text-slate-700 md:text-base">
                <li>
                  <span className="font-semibold">{t("이 비자는 이런 분께 맞아요", "Best for", "适合人群", "Phù hợp với")}:</span>{" "}
                  {pickVisaApplicant(visa)}
                </li>
                <li>
                  <span className="font-semibold">{t("보통 체류기간", "Typical stay", "常见停留期限", "Thời gian lưu trú phổ biến")}:</span>{" "}
                  {pickVisaDuration(visa)}
                </li>
                <li>
                  <span className="font-semibold">{t("준비 포인트", "What to prepare", "准备要点", "Điểm cần chuẩn bị")}:</span>{" "}
                  {t(
            ...
            <section>
              <h3 className="text-sm font-black text-[#0B1227] md:text-base">{t("상세 설명", "Detailed Description", "详细说明", "Mô tả chi tiết")}</h3>
              {descriptionLines.length
                ...
                : (
                  <div className="mt-2 space-y-1 text-sm text-slate-700 md:text-base">
                    ...
                    <p>
                      {t("체류기간 기준", "Stay period reference", "停留期限参考", "Mốc tham chiếu thời gian lưu trú")}: {pickVisaDuration(visa)}
                    </p>
                  </div>
                )}
            </section>
            ...
            <section>
              <h3 className="text-sm font-black text-[#0B1227] md:text-base">{t("누가 신청할 수 있나요?", "Who can apply?", "谁可以申请？", "Ai có thể nộp hồ sơ?")}</h3>
            ...
            <section>
              <h3 className="text-sm font-black text-[#0B1227] md:text-base">{t("준비해야 할 서류", "Required Documents", "所需材料", "Hồ sơ cần chuẩn bị")}</h3>
              {requirementLines.length
                ? renderStructured(requirementLines)
                : (
                  <div className="mt-2 space-y-1 text-sm text-slate-700 md:text-base">
                    <p>
                      {t("여권", "Passport", "护照", "Hộ chiếu")}
                    </p>
                    <p>
                      {t("비자 신청서", "Visa application form", "签证申请表", "Đơn xin visa")}
                    </p>
                    <p>
                      {t("사진", "ID photo", "证件照", "Ảnh thẻ")}
                    </p>
                    <p>
                      {t("추가 증빙서류(자격별 상이)", "Additional supporting documents (varies by status)", "附加证明材料（因资格而异）", "Hồ sơ bổ sung (khác nhau theo từng diện)")}
                    </p>
                  </div>
                )}
            </section>
            ...
            <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
              <h3 className="text-sm font-black text-[#0B1227] md:text-base">{t("주의할 점", "What to watch out for", "注意事项", "Lưu ý")}</h3>
              <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-700 md:text-base">
                <li>{t("같은 코드라도 국적, 학력, 경력, 체류이력에 따라 심사 결과가 달라질 수 있어요.", "Even with the same visa code, outcomes can differ by nationality, education, career, and stay history.", "即使同一代码，审核结果也可能因国籍、学历、经历和居留记录而不同。", "Dù cùng mã visa, kết quả xét duyệt vẫn có thể khác theo quốc tịch, học vấn, kinh nghiệm và lịch sử cư trú.")}</li>
                <li>{t("서류 발급일·번역·공증 요건은 제출 직전에 다시 확인해 주세요.", "Please re-check document issue dates, translation, and notarization requirements right before submission.", "请在提交前再次确认材料签发日期、翻译和公证要求。", "Hãy kiểm tra lại ngày cấp giấy tờ, yêu cầu dịch thuật và công chứng ngay trước khi nộp.")}</li>
                <li>{t("체류기간과 취업 가능 범위는 허가 내용 기준으로 최종 확정돼요.", "Stay period and work scope are finalized by the actual permit details.", "停留期限和可就业范围以最终获批内容为准。", "Thời gian lưu trú và phạm vi được phép làm việc sẽ được chốt theo nội dung cấp phép cuối cùng.")}</li>
              </ul>
            </section>
          </div>
        </div>
