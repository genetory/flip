...
export function VisaResourcesPage() {
  ...
  const pickGroupTitle = (item: FlatVisaItem) =>
    locale === "ko" ? item.groupTitleKo : locale === "zh-CN" ? item.groupTitleZh : locale === "vi" ? item.groupTitleVi : item.groupTitleEn;
  ...
  const filtered = useMemo(() => {
    ...
    return flattenedItems.filter((item) => {
      ...
      const text = `${item.code} ${item.labelKo} ${item.labelEn} ${item.labelZh} ${item.labelVi} ${item.applicantKo} ${item.applicantEn} ${item.applicantZh} ${item.applicantVi}`.toLowerCase();
    ...
    >
      <section className="rounded-3xl border border-slate-200 bg-white p-6 md:p-7">
        <h2 className="text-xl font-extrabold text-[#0B1227] md:text-2xl">
          {t("모든 비자 유형", "All Visa Types", "全部签证类型", "Tất cả các loại visa")}
        </h2>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {[
            {
              key: "all",
              label: t("전체 비자", "All Visas", "全部签证", "Tất cả visa"),
              labelMobile: t("전체", "All Visas", "全部", "Tất cả")
            },
            {
              key: "work",
              label: t("취업 목적", "Work Purpose", "就业目的", "Mục đích việc làm"),
              labelMobile: t("취업", "Work Purpose", "就业", "Việc làm")
            },
            {
              key: "study",
              label: t("학업 목적", "Study Purpose", "学业目的", "Mục đích du học"),
              labelMobile: t("학업", "Study Purpose", "学业", "Du học")
            },
            {
              key: "residence",
              label: t("거주 목적", "Residence Purpose", "居住目的", "Mục đích cư trú"),
              labelMobile: t("거주", "Residence Purpose", "居住", "Cư trú")
            },
            {
              key: "travel",
              label: t("여행/방문", "Travel / Visit", "旅游 / 访问", "Du lịch / Thăm thân"),
              labelMobile: t("여행/방문", "Travel / Visit", "旅游 / 访问", "Du lịch")
            },
            {
              key: "other",
              label: t("기타 목적", "Other Purpose", "其他目的", "Mục đích khác"),
              labelMobile: t("기타", "Other Purpose", "其他", "Khác")
            }
          ].map((tab) => {
          ...
          {filtered.map((item) => (
            ...
            >
              ...
              <p className="mt-1 text-xs text-slate-600 md:text-sm">
                <span className="font-semibold">{t("신청대상", "Applicants", "申请对象", "Đối tượng nộp hồ sơ")}:</span> {pickApplicant(item)}
              </p>
              <p className="text-xs text-slate-600 md:text-sm">
                <span className="font-semibold">{t("체류기간", "Stay period", "停留期限", "Thời gian lưu trú")}:</span> {pickDuration(item)}
              </p>
            </Link>
          ))}
          {filtered.length === 0 ? (
