...
export function ResumeScoreCard() {
  ...
  const fetchScore: ScoreFetch = async (opts) => {
    ...
    return {
      view: {
        ...
        why: r.score.why,
        sections: [
          ...(r.score.strong?.length ? [{ title: t("잘한 점", "Strengths", "做得好", "Điểm mạnh", "良い点", "Kelebihan"), tone: "good" as const, items: r.score.strong }] : []),
          ...(r.score.tips.length ? [{ title: t("우선 고칠 점", "Fix these first", "优先改进", "Ưu tiên sửa", "まず直す点", "Perbaiki dulu"), tone: "info" as const, items: r.score.tips }] : [])
        ]
      },
      stale: r.stale,
  ...
  return (
    <ScoreCard
      ...
      editLabel={t("이력서 수정", "Edit resume", "修改简历", "Sửa CV", "履歴書を修正", "Edit resume")}
    />
  );
}
