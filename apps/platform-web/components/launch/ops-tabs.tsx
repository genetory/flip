...
export function OpsTabs() {
  ...
  const TABS = [
    { href: "/career-launch/ops", label: t("홈", "Home", "首页", "Trang chủ", "ホーム", "Beranda"), exact: true },
    ...
    <div className="border-b border-[#EEF1F5] bg-white/90 backdrop-blur">
      <LaunchContainer className="!max-w-6xl">
        <div className="flex h-12 items-center justify-between">
          <Link href="/career-launch/ops" className="text-[13px] font-extrabold text-[#0B1227] transition hover:text-[#0B46E8]">{t("Launch 운영", "Launch admin", "Launch 运营", "Quản trị Launch", "Launch 運営", "Admin Launch")}</Link>
        ...
        <nav className="flex gap-1 overflow-x-auto pb-2">
          {TABS.map((t) => {
            const active = "exact" in t && t.exact ? pathname === t.href : pathname === t.href || pathname.startsWith(`${t.href}/`);
            return (
              <Link
                key={t.href}
                href={t.href}
