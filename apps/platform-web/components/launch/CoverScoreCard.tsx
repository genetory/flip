...
export function CoverScoreCard() {
  ...
  const fetchScore: ScoreFetch = async (opts) => {
    ...
    if (r.score.strong?.length) sections.push({ title: t("잘한 점", "Strengths", "做得好", "Điểm mạnh", "良い点", "Kelebihan"), tone: "good", items: r.score.strong });
    ...
    if (r.score.tips.length) sections.push({ title: t("우선 고칠 점", "Fix these first", "优先改进", "Ưu tiên sửa", "まず直す点", "Perbaiki dulu"), tone: "info", items: r.score.tips });
    return {
      view: {
        total: r.score.total,
        breakdown: [
  ...
  return (
    <ScoreCard
      ...
      editLabel={t("자기소개서 수정", "Edit cover letter", "修改自我介绍", "Sửa thư", "自己紹介書を修正", "Edit surat lamaran")}
    />
  );
}
