...
export function WeekGate({ week, children }: { week: number; children: React.ReactNode }) {
  ...
  if (state === "loading") return <Card className="!p-8 text-center text-[13px] text-[#8B95A1]"><span className="inline-flex items-center gap-2"><CircleNotch className="h-4 w-4 animate-spin" weight="bold" aria-hidden /> {t("불러오는 중…", "Loading…", "加载中…", "Đang tải…", "読み込み中…", "Memuat…")}</span></Card>;
  if (state === "locked") {
    ...
    return (
      <Card className="!p-8 text-center">
        <span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#F2F4F6] text-[#8B95A1]"><Lock className="h-6 w-6" weight="fill" aria-hidden /></span>
        <p className="mt-3 text-[15px] font-black text-[#191F28]">{t("아직 잠겨 있어요", "Still locked", "尚未解锁", "Vẫn đang khóa", "まだロックされています", "Masih terkunci")}</p>
        {dateLocked ? (
          ...
          <>
            ...
            <Link href={`/career-launch/week/${week - 1}`} className="mt-4 inline-flex items-center justify-center gap-1.5 rounded-xl bg-[#0B46E8] px-5 py-2.5 text-[13.5px] font-bold text-white transition hover:bg-[#0A3ECB]">
              {t(`Week ${week - 1} 이어서 하기`, `Continue Week ${week - 1}`, `继续 Week ${week - 1}`, `Tiếp tục Week ${week - 1}`, `Week ${week - 1} を続ける`, `Lanjutkan Week ${week - 1}`)} <ArrowRight className="h-4 w-4" weight="bold" aria-hidden />
            </Link>
          </>
        )}
      </Card>
