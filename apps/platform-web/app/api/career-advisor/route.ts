...
이 정리 텍스트는 reply 에 담고, 추가로 정리 단계에서는 방금 제시한 어울리는 포지션을 [직무 후보 목록]에서 골라 jobCategories 에 1~3개 담아라(그래야 사용자에게 그 직무의 실제 공고를 바로 보여줄 수 있다). jobCategories 값은 [직무 후보 목록]과 글자까지 정확히 일치해야 하고, 목록에 없는 직무는 넣지 마라. 탐색이 아직 무르익지 않은 단계에서는 jobCategories 를 빈 배열로 둬라.
...
반드시 JSON 객체로만 답해: {"reply": "사용자에게 건네는 말", "jobCategories": ["직무 후보 목록에서 고른 어울리는 직무명(정리 단계에서만 1~3개, 그 외에는 빈 배열)"]}`;
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { reply?: string; jobCategories?: unknown };

    const reply = typeof parsed.reply === "string" ? parsed.reply.trim() : "";
    if (!reply) return NextResponse.json({ error: "empty" }, { status: 502 });
    // 정리 단계에서 고른 직무 — 후보 풀에 있는 값만(실제 공고 매핑용). 최대 3개.
    const poolSet = new Set(pool);
    const jobCategories = Array.isArray(parsed.jobCategories)
      ? Array.from(new Set(parsed.jobCategories.filter((x): x is string => typeof x === "string").map((x) => x.trim()).filter((x) => poolSet.has(x)))).slice(0, 3)
      : [];
    return NextResponse.json({ reply, jobCategories });
  } catch {
