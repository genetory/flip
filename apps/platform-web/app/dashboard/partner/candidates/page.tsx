  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-5 py-6 md:py-8">
      <header className="mb-5">
        <h1 className="text-[22px] font-black tracking-[-0.02em] text-[#0B1227]">인재 검색</h1>
        <p className="mt-1 text-[13.5px] text-[#4E5968]">인재풀 등록에 동의한 후보자의 대표 이력서를 검색하고, 관심 있는 인재에게 연결을 요청하세요. 연락처는 후보자가 수락하면 공개됩니다.</p>
      </header>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 flex items-center gap-1 truncate text-[12.5px] text-[#8B95A1]">
                      {c.nationality ? <><MapPin className="h-3.5 w-3.5" />{c.nationality}</> : null}
                    </p>
                  </div>
      ...
      {detailId ? (
        <CandidateDetailModal
          candidateUserId={detailId}
          onClose={() => setDetailId(null)}
          onStatusChange={(s) => patchStatus(detailId, s)}
        />
      ) : null}
    </main>
  );
            ...
            {!status ? (
              <div className="space-y-2">
                <textarea
                  ...
                  rows={2}
                  placeholder="후보자에게 전할 메시지 (선택) — 우리 회사·포지션을 소개해보세요"
                  className="w-full resize-none rounded-xl border border-[#E5E8EB] px-3.5 py-2.5 text-[13.5px] outline-none focus:border-[#0B46E8]"
                />
