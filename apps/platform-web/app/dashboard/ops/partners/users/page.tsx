...
import { OpsBadge, toneFromEmailVerified, toneFromPartnerOrgRole } from "../_components/OpsBadge";
import { PartnerUnifiedDetailModal } from "../_components/PartnerUnifiedDetailModal";
...
export default function PartnerUsersPage() {
  ...
  // Server-side filters. Same shape / "ALL" sentinel as the all-users page.
  // `role` is hard-coded PARTNER on the backend so it's not exposed here —
  // partnerOrgRole (소유자/관리자/멤버) is the meaningful "역할" instead.
  ...
  const [partnerOrgRoleFilter, setPartnerOrgRoleFilter] = useState<"ALL" | "OWNER" | "ADMIN" | "MEMBER">("ALL");
  ...
  async function fetchPartnerUsers() {
    ...
    try {
      ...
      if (partnerOrgRoleFilter !== "ALL") params.set("partnerOrgRole", partnerOrgRoleFilter);
      ...
      if (!response.ok || !payload.ok) {
        setListErrorMessage(payload.message ?? "파트너 사용자 목록을 불러오지 못했습니다.");
        return;
      }
  ...
  }, [debouncedSearch, sortField, sortOrder, page, pageSize, emailVerifiedFilter, authProviderFilter, partnerOrgRoleFilter]);
  ...
  async function saveUserMemo() {
    ...
    try {
      const token = readCookie(TOKEN_COOKIE_KEY);
      const response = await fetch(`${apiBaseUrl}/ops/partner-users/${selectedUser.id}/admin-memo`, {
        method: "PATCH",
        headers: {
  ...
  // 운영진이 이메일 인증을 직접 승인/해제(파트너 유저도 User 레코드라 /ops/users 엔드포인트 공용).
  ...
  return (
    <section className="ops-content-section">
      <header>
        <h1>파트너 사용자 관리</h1>
        <p>파트너 담당자 계정을 조회하고 검색/정렬/페이징으로 운영합니다.</p>
      </header>
      ...
      <article className="ops-partner-list-card">
        ...
        <div className="ops-partner-filters ops-partner-filters--multi">
          ...
          />
          <select
            value={partnerOrgRoleFilter}
            onChange={(e) => {
              setPartnerOrgRoleFilter(e.target.value as typeof partnerOrgRoleFilter);
              setPage(1);
            }}
            aria-label="역할 필터"
          >
          ...
          <table className="ops-partner-table ops-partner-users-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="ops-table-empty">조건에 맞는 파트너 사용자가 없습니다.</td>
                </tr>
              ) : (
                    ...
                    <div className="ops-detail-sections">
                      <section className="ops-detail-section">
                        ...
                        <div className="ops-detail-grid">
                          ...
                          <div><span>권한</span><strong>파트너</strong></div>
                          <div>
                            <span>파트너 역할</span>
                            <strong>
